...
TEST_F(BookStoreTest, AddBookSuccess) {
  ...
  EXPECT_NO_THROW(
      store->addBook(Book("Another Book", "Another Author", 2024, 19.99)));
}

TEST_F(BookStoreTest, RemoveBookSuccess) {
  store->addBook(Book("Test Title", "Test Author", 2024, 29.99));
  EXPECT_NO_THROW(store->removeBook(
      "Test Title")); // Убедитесь, что книга может быть удалена
}

TEST_F(BookStoreTest, FindBookSuccess) {
  store->addBook(Book("Test Title", "Test Author", 2024, 29.99));
  EXPECT_NO_THROW(
      store->findBook("Test Title")); // Убедитесь, что книга найдена
}
