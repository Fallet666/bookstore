...
void BookStore::createTable() const {
  ...
  if (sqlite3_exec(db, setEncoding, nullptr, nullptr, &errMsg) != SQLITE_OK) {
    std::string errorMessage =
        "Ошибка установки кодировки: " + std::string(errMsg);
    sqlite3_free(errMsg);
  ...
  if (sqlite3_exec(db, sql, nullptr, nullptr, &errMsg) != SQLITE_OK) {
    std::string errorMessage =
        "Ошибка создания таблицы: " + std::string(errMsg);
    sqlite3_free(errMsg);
    throw std::runtime_error(errorMessage);
  }
}
...
void BookStore::addBook(const Book &book) const {
  ...
  if (sqlite3_step(stmt) != SQLITE_DONE) {
    ...
    throw std::runtime_error("Ошибка добавления книги: " +
                             std::string(sqlite3_errmsg(db)));
  }
  sqlite3_finalize(stmt);
}
...
void BookStore::removeBook(const std::string &title) const {
  ...
  if (sqlite3_step(stmt) != SQLITE_DONE) {
    sqlite3_finalize(stmt);
    throw std::runtime_error("Ошибка удаления книги: " +
                             std::string(sqlite3_errmsg(db)));
  }
